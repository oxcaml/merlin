...
module Evaluation_cache = Identifiable.Make (struct
  type t = Shape.t
  ...
  let output __ _ = Misc.fatal_error "unimplemented"
...
let closed_cache = Evaluation_cache.Tbl.create 256
...
let add_to_cache t res subst_type (subst_constr_mut, subst_constr) =
  ...
  then Evaluation_cache.Tbl.add closed_cache t res

let find_in_cache t = Evaluation_cache.Tbl.find_opt closed_cache t
      ...
      let result =
        ...
        | None -> (
          ...
          | App (f, arg) -> (
            let f =
              unfold_and_evaluate ~diagnostics ~depth ~steps_remaining
                subst_type subst_constr f
            in
            let arg =
              unfold_and_evaluate ~diagnostics ~depth ~steps_remaining
                subst_type subst_constr arg
            in
          ...
          | Variant_unboxed
            ...
            Shape.variant_unboxed ~variant_uid ~arg_uid name arg_name
              (unfold_and_evaluate ~diagnostics ~depth ~steps_remaining
                 subst_type subst_constr arg_shape)
              arg_layout
          | Proj (t, i) ->
            Shape.proj
              (unfold_and_evaluate ~diagnostics ~depth ~steps_remaining
                 subst_type subst_constr t)
              i
          | Tuple args ->
            Shape.tuple
              (List.map
