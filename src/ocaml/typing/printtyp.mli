...
val wrap_printing_env:
  ?verbosity:'a -> ?error:bool -> Env.t -> (unit -> 'b) -> 'b
(** Merlin-compatible wrapper around {!Out_type.wrap_printing_env}.
    [verbosity] is accepted for compatibility with Merlin call sites. *)
...
val tree_of_type_scheme: type_expr -> Outcometree.out_type
val tree_of_type_declaration:
  Ident.t -> type_declaration -> rec_status -> Outcometree.out_sig_item
val tree_of_modtype: ?abbrev:bool -> module_type -> Outcometree.out_module_type
val tree_of_modtype_declaration:
  ?abbrev:bool -> Ident.t -> modtype_declaration -> Outcometree.out_sig_item
val tree_of_modalities:
  mutability -> Mode.Modality.Const.t -> Outcometree.out_mode list

val type_declaration_for_merlin:
  print_non_value_inferred_jkind:bool ->
  Ident.t -> Format.formatter -> type_declaration -> unit

val type_scheme_for_merlin:
  print_non_value_jkind_on_type_variables:bool ->
  Format.formatter -> type_expr -> unit
